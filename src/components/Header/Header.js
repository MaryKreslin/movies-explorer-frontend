...
const Header = (props) => {
    ...
    React.useEffect(() => {
        ...
        if (props.type === "movies" || "savedMovies") {
    ...
    return (
        <header className={`header header_${props.type}`}>
            ...
            <div className='header__navigation-block'>
                <p className={`header__navigation-text ${props.type === 'movies' ? 'header__navigation-text_active' : ''}`}>{header.text1}</p>
                <p className={`header__navigation-text ${props.type === 'savedMovies' ? 'header__navigation-text_active' : ''}`}>{header.text2}</p>
            </div>
            <div className='header__profile-block'>
                ...
                <button className={`header__button header__button_type_${props.type}`}>
                    ...
                    {(props.type === 'movies' || props.type === 'savedMovies') && <img className='header__buttonImage' src={icon} alt='Иконка аккаунта' />}
                </button>
            </div>
        </header>
    )
